CREATE OR REPLACE VIEW `mlwh_reporting`.`sample_tracking_view` AS

WITH sample_events AS (
    SELECT event_type, occured_at, subject_uuid_bin FROM `mlwh_events`.flat_events_view
...
FROM `mlwh_reporting`.sample_flowcell_view
